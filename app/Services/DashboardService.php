...
class DashboardService
{
    ...
    {
        return Cache::remember('dashboard.admin', 300, function () {
            ...
            $recentProjects = $this->project->withCount([
                    'tasks',
    ...
    {
        return Cache::remember("dashboard.pm.{$userId}", 180, function () use ($userId) {
            $managedProjects = $this->project->byManager($userId)->withCount([
                'tasks',
                'tasks as completed_tasks_count' => fn($q) => $q->where('status', 'completed'),
            ])->get();
            ...
            $employeeProductivity = $this->user->whereHas('assignedTasks', function ($q) use ($managedProjectIds) {
    ...
    {
        return Cache::remember("dashboard.emp.{$userId}", 180, function () use ($userId) {
